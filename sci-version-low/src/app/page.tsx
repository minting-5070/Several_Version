...
export default function Home() {
  ...
  // 로그인 페이지로 자동 리다이렉트하지 않음 — 최초 방문 시 모달을 표시

  // 브랜드 로고 프리로드 (모달 워터마크 및 소형 마크 공용)
  useEffect(() => {
  ...
  const handleSubmitGuarded = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      (window as any).dataLayer.push({
        ...
        app_version: 'sci-version-low'
      });
    } catch {}
    handleSubmit(e);
  };
  
  // Marketing banner (first-visit modal ≈ login)
  ...
  useEffect(() => {
    try {
      ...
      // Always show if no Prolific ID (acts as login), or when forced via query string
      if (forceShow || !storedId) setShowNotice(true);
    } catch {}
  }, []);
  const dismissNotice = () => {
    try { localStorage.setItem('cvt_marketing_seen_ra_v2', '1'); } catch {}
    setShowNotice(false);
  };
  ...
  // Emit event when a new assistant answer arrives (post-stream)
              ...
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                ...
                <div className="relative z-20 px-6 py-7 bg-gradient-to-br from-white/95 via-violet-50/85 to-blue-50/80 dark:from-neutral-900/90 dark:via-violet-900/40 dark:to-blue-900/40 ring-1 ring-white/30">
                  ...
                  <div className="mt-7 flex items-center justify-end gap-3">
                    ...
                    <button onClick={saveProlificAndDismiss} disabled={!prolificInput.trim()} className={`${prolificInput.trim() ? 'bg-primary text-primary-foreground hover:bg-primary/90' : 'bg-muted text-muted-foreground cursor-not-allowed'} px-5 py-2.5 text-sm md:text-base rounded-lg`}>Start now</button>
                  </div>
                  </div>
                </div>
              </div>
